...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const postAddress = await axios.post(`${config.endpoint}/user/addresses`, 
      {
      ...
      setAddresses({...addresses, all: postAddress.data});
      setNewAddress({...newAddress, isAddingNewAddress: false, value: "" });
  ...
  const deleteAddress = async (token, addressId) => {
    ...
    try {
      ...
      const deleteAddress = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`, 
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      setAddresses({...addresses, all: deleteAddress.data});
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    // console.log(token, items, addresses);
              ...
              { addresses.all.length ? 
                ( addresses.all.map((address) => 
                  ...
                  className=
                    ...
                    <Button startIcon={<Delete/>} onClick={() => deleteAddress(token, address._id)}>DELETE</Button>
                  </Box>
                )) :
                <Typography my="1rem">
